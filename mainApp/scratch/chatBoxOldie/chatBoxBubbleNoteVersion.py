...
from mainApp.scratch.chatBoxOldie.bubbleNote import MessageModel, MessageDelegate
...
class ChatBox(QWidget):
    ...
    def initUI(self):
        ...
        self.chatBox = QListView()
        self.chatBox.setItemDelegate(MessageDelegate())
        self.chatBox.setModel(self.model)
    ...
    def addQuestion(self, message):
        self.model.addMessage("User", f"{message}\n")
        self.refreshChat()

    def addAnswerFromChatBot(self, message):
        self.model.addMessage("Chat", f"{message}\n")
        self.refreshChat()

    def addAnswerFromSystem(self, message):
        self.model.addMessage("System", f"{message}\n")

    def refreshChat(self):
        self.chatBox.update()
        self.questionBox.setPlainText("")
        self.questionBox.moveCursor(QTextCursor.Start)
